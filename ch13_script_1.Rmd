...
> For males compared to females, the upper bound in the change in estimate gvote decreases by 3%.

> For one unit increase in in income, the upper bound in the likelihood of voting democrat decreases by 7%
...
> Bayesian interpretation: We are 95% confident that the true coefficients of the effect of income on the likelihood of voting democrat is in the confident interval.

> Frequentist interpretation: If we generate infintiely many of these intervals we would capture the true coefficient 95% of the time
...
> The higher someone's income, the less likely they will vote for a democratic candidate
...
> In this case, the estimate for Intercept would not have a meaningful interpretation because there is no value for what it means when income = 0. However, if it did have an interpretable meaning, we would say that the likelihood of someone voting democrat with gendermale = 0 and income = 0 is 18%.
...
```{r}
augment(x = model_1, data = nes, type.predict = "response")
```


What does the `.fitted` column means?

> The fitted column is the predicted likelihood of voting democrat for each individual based on their gender and income.
...
```{r}
augment(x = model_1, data = nes, type.predict = "response") %>%
  filter(income == 3 & gender == "male")
```

> All males with income 3 will have the same value because those are the only two input variables.
